...
let perimeter (s : shape) = match s with 
  | Circ (r, (cx, cy)) -> 2. *. 3.14 *. r
  | Rect ((x1, y1), (x2, y2)) -> 2. *. ((x2 -. x1) +. (y2 -. y1));;

type direction = N | NE | E | SE | S | SW | W | NW;;

(* New Opposite Function *)
let newOpposite (d : direction) : direction = match d with 
  | N -> S
  | S -> N
  | E -> W
  | W -> E
...
type newCoord = int * int;;

let newMove (c : newCoord) (d : direction) : newCoord = 
  match c with (cx, cy) ->
    match d with 
    | N  -> (cx + 0, cy + 1)
    | E  -> (cx + 1, cy + 0)
